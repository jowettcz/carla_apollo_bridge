...
class EgoVehicle(Vehicle):
    ...
    def send_vehicle_msgs(self, frame, timestamp):
        ...
        vehicle_chassis.speed_mps = self.get_vehicle_speed_abs(
            self.carla_actor)
        vehicle_chassis.throttle_percentage = self.carla_actor.get_control().throttle * \
            100.0
        vehicle_chassis.brake_percentage = self.carla_actor.get_control().brake * 100.0
        vehicle_chassis.steering_percentage = -self.carla_actor.get_control().steer * \
            100.0
        ...
        if not self.vehicle_info_writed:
            ...
            for wheel in vehicle_physics.wheels:
                ...
                inv_T = np.array(self.carla_actor.get_transform(
                ).get_inverse_matrix(), dtype=float)
        ...
        if not self.vehicle_loc_set:
            ...
            self.node.loginfo("location is {}, rotation is {}".format(
            ...
            cyber_twist = trans.carla_velocity_to_cyber_twist(
            ...
            roll, pitch, yaw = trans.cyber_quaternion_to_cyber_euler(
                cyber_pose.orientation)
            # self.node.loginfo(
            #     "====roll is {}, pitch is {}, yaw is {}".format(roll, pitch, yaw))
            ...
            localization_estimate.pose.heading = math.radians(
                -transform.rotation.yaw)
    ...
    def control_command_updated(self, cyber_vehicle_control, manual_override):
        ...
        :return:
        """
        print("control_command_updated {}".format(self.carla_actor))
        if self.carla_actor is None:
            return
        if manual_override == self.vehicle_control_override:
            print("Get control command from manual(movex), current control mode is {}".format(self.vehicle_control_override))
            ...
            # except Exception as e:
                #     print("carla actor maybe destroyed, ignore it. {}".format(str(e)))
    ...
    def enable_autopilot_updated(self, enable_auto_pilot):
        ...
        """
        self.node.logdebug(
            "Ego vehicle: Set autopilot to {}".format(enable_auto_pilot.value))
